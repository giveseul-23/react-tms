...
export function usePodColectionReportController({ model }: Args) {
  ...
  const loadSub01 = useCallback(
    async (mainRow: any) => {
      const rows = await base.searchSub("sub01", api.getSub01List(sub01Params(mainRow)));
      base.resetGrids(["sub02"]);
      const first = rows?.[0];
      if (first) {
        model.grids.sub01.setSelected(first);
        void base.searchSub("sub02", api.getSub02List(sub02Params(first)));
      }
    },
  ...
  const onSearchCallback = useCallback(
    ...
    [base, model.grids.main, loadSub01],
  );

  // 메인 클릭 → sub01 조회 (sub02 reset)
  const onMainGridClick = useCallback(
    (row: any) => {
      base.handleRowClick("main", row, undefined, { alsoReset: ["sub01", "sub02"] });
      if (row) void loadSub01(row);
    },
    [base, loadSub01],
  );
  ...
  const sub01Actions: ActionItem[] = useMemo(
    () => [
      makeExcelGroupAction({
        ...
        menuCode: MENU_CODE,
        menuName,
        fetchFn: () => api.getSub01List(sub01Params(model.grids.main.selectedRef.current)),
        rows: model.grids.sub01.rows,
      }),
    ],
    [menuName, model.grids.sub01, model.grids.main.selectedRef],
  );
  ...
  const sub02Actions: ActionItem[] = useMemo(
    () => [
      makeExcelGroupAction({
        ...
        menuCode: MENU_CODE,
        menuName,
        fetchFn: () => api.getSub02List(sub02Params(model.grids.sub01.selectedRef.current)),
        rows: model.grids.sub02.rows,
      }),
    ],
    [menuName, model.grids.sub02, model.grids.sub01.selectedRef],
  );
