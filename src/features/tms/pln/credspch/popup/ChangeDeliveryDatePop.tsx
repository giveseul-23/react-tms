...
export default function ChangeDeliveryDatePop({
  ...
  const dpDlvyDt = Util.formatDttm(initialValues.DLVRY_DT);
  const [dlvryDt, setDlvryDt] = useState(dpDlvyDt ?? "");
    ...
    if (!list.some((item: any) => item.CODE === planId)) {
      setPlanId(
        initialValues.PLN_ID &&
          list.some((i: any) => i.CODE === initialValues.PLN_ID)
          ? initialValues.PLN_ID
          : list[0].CODE,
      );
    }
  ...
  return (
    <FormPopupLayout
      ...
      onCancel={onClose}
      onConfirm={() =>
        onConfirm({ DLVRY_DT: toCompactDate(dlvryDt), PLN_ID: planId })
      }
    >
      <Field
        layout="vertical"
      ...
      <div>
        ...
        <DatePickerPopover value={dlvryDt} onChange={setDlvryDt} size="lg" />
      </div>
    </FormPopupLayout>
  );
}
