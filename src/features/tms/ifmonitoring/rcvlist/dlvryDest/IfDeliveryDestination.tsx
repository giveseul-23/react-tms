...
import { useFeatureModel } from "./IfDeliveryDestinationModel";
import { useFeatureController } from "./IfDeliveryDestinationController";
...
// TODO: 실제 메뉴 코드로 교체
export const MENU_CODE = "MENU_IF_RCV_DLVRY_DEST";

// 서버 리소스 권한 authId (센차 grid.authId). 그리드별 authId 단일 소스.
// DataGrid authId(그리드 리소스 권한) + Controller 엑셀 gridId(업로드/양식) 가 이걸 참조.
export const AUTH = {
  grids: {
    main: "MAIN_GRID_IF_RCV_DLVRY_DEST",
    role_type: "DETAIL_GRID_IF_RCV_DLVRY_DEST1",
    sales: "DETAIL_GRID_IF_RCV_DLVRY_DEST2",
    addr: "DETAIL_GRID_IF_RCV_DLVRY_DEST3",
    bank_account: "DETAIL_GRID_IF_RCV_DLVRY_DEST4",
    company: "DETAIL_GRID_IF_RCV_DLVRY_DEST5",
  },
};

export default function IfDeliveryDestination() {
    ...
    <MasterDetailPage
      ...
      detail={
        <DataGrid
          layoutType="tab"
          tabs={[
            { key: "ROLE_TYPE", label: "LBL_ROLE_TYPE" },
            { key: "SALES", label: "LBL_SALES" },
            { key: "ADDR", label: "LBL_ADDR" },
            { key: "BANK_ACCOUNT", label: "LBL_BANK_ACCOUNT" },
          ...
          presets={{
            ROLE_TYPE: {
              render: () => (
                <DataGrid
                  {...model.bind("role_type")}
                  authId={AUTH.grids.role_type}
                  columnDefs={ROLE_TYPE_COLUMN_DEFS}
                  codeMap={model.codeMap}
                  actions={ctrl.tabActions.role_type}
                  audit={false}
                  pagination={false}
                />
              ),
            ...
            SALES: {
              render: () => (
                <DataGrid
                  {...model.bind("sales")}
                  authId={AUTH.grids.sales}
                  columnDefs={SALES_COLUMN_DEFS}
                  codeMap={model.codeMap}
                  actions={ctrl.tabActions.sales}
                  audit={false}
                  pagination={false}
                />
              ),
            ...
            ADDR: {
              render: () => (
                <DataGrid
                  {...model.bind("addr")}
                  authId={AUTH.grids.addr}
                  columnDefs={ADDR_COLUMN_DEFS}
                  codeMap={model.codeMap}
                  actions={ctrl.tabActions.addr}
                  audit={false}
                  pagination={false}
                />
              ),
            ...
            BANK_ACCOUNT: {
              render: () => (
                <DataGrid
                  {...model.bind("bank_account")}
                  authId={AUTH.grids.bank_account}
                  columnDefs={BANK_ACCOUNT_COLUMN_DEFS}
                  codeMap={model.codeMap}
                  actions={ctrl.tabActions.bank_account}
                  audit={false}
                  pagination={false}
                />
              ),
            ...
            COMPANY: {
              render: () => (
                <DataGrid
                  {...model.bind("company")}
                  authId={AUTH.grids.company}
                  columnDefs={COMPANY_COLUMN_DEFS}
                  codeMap={model.codeMap}
                  actions={ctrl.tabActions.company}
                  audit={false}
                  pagination={false}
                />
              ),
