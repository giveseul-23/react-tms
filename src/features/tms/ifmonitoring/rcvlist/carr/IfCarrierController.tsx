...
import { ifCarrierApi as api } from "./IfCarrierApi";
import { MENU_CODE } from "./IfCarrier";
import type { ActionItem } from "@/app/components/ui/GridActionsBar";
import type { GridKey, IfCarrierModel } from "./IfCarrierModel";

interface Args {
  model: IfCarrierModel;
}
...
export function useIfCarrierController({ model }: Args) {
  ...
  const onMainGridClick = useCallback(
    (row: any) => {
      ...
      base.handleRowClick("main", row, [
        {
          to: "sub01",
          fetch: (r) => api.searchBank(buildSubParams(r)),
        },
        {
          to: "sub02",
          fetch: (r) => api.searchComp(buildSubParams(r)),
        },
      ]);
    },
    [base],
