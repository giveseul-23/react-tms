...
const getRows = (res: any): any[] => {
  const rows = res?.data?.data?.dsOut ?? res?.data?.result ?? res?.data?.data ?? [];
...
const toPoints = (rows: any[]): TracePoint[] =>
  rows
    .map((row) => ({
...
const toStops = (rows: any[]): RouteStop[] =>
  rows.map((row, index) => {
    ...
    return {
      id: `route-stop-${row.STOP_ID ?? row.LOC_ID ?? index}`,
      lat: toNumber(row.LAT),
      lon: toNumber(row.LON),
...
export default function RouteMapPopup({ row }: Props) {
  const mapRef = useRef<TmapViewHandle | null>(null);
  const expectedLineRef = useRef<any>(null);
  ...
  const clearExpectedLine = useCallback(() => {
    if (expectedLineRef.current) {
      expectedLineRef.current.setMap(null);
      expectedLineRef.current = null;
    }
  }, []);
  ...
  const drawExpectedLine = useCallback(
    (points: TracePoint[]) => {
      clearExpectedLine();
      ...
      const Tmapv2 = window.Tmapv2;
      const valid = points.filter(isValidPoint);
      if (!map || !Tmapv2 || valid.length < 2) return;

      expectedLineRef.current = new Tmapv2.Polyline({
        path: valid.map((point) => new Tmapv2.LatLng(point.lat, point.lon)),
  ...
  const draw = useCallback(async () => {
    ...
    try {
      const [routeRes, expectedRes, traceRes] = await Promise.all([
        api.getDlvryRoute({ DSPCH_NO: dspchNo }),
        api.getExpectedRoute({ DSPCH_NO: dspchNo }),
        api.searchDispatchTrace({ DSPCH_NO: dspchNo }),
      ]);

      for (const res of [routeRes, expectedRes, traceRes]) {
        if (res?.data?.success === false) {
          setError(res.data.msg ?? Lang.get("MSG_FAIL"));
          return;
        }
      }

      const routeRows = getRows(routeRes);
      const routeStops = toStops(routeRows);
      const tracePoints = toPoints(getRows(traceRes));
      const expectedPoints = toPoints(getRows(expectedRes).slice(1));

      setRouteStops(routeStops);
      setIsStopListExpanded(routeStops.length < 5);
      drawRouteMarkers(routeStops);
      drawExpectedLine(expectedPoints);
  ...
  useEffect(
    ...
    [clearExpectedLine, clearRouteMarkers],
  );
