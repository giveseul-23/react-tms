...
export function useTempOilPriceModel() {
  // ── 레이아웃 ──────────────────────────────────────────────────
  ...
  // ── 탭 A: 유가등록 ────────────────────────────────────────────
  // 좌측 — 물류운영그룹 목록
  const [masterRowData, setMasterRowData] = useState<GridData>(EMPTY_GRID);
  // 우측 — 선택된 그룹의 유가 상세
  const [oilPriceRowData, setOilPriceRowData] = useState<GridData>(EMPTY_GRID);

  // ── 탭 B: 기간별조회 ──────────────────────────────────────────
  const [periodRowData, setPeriodRowData] = useState<GridData>(EMPTY_GRID);
