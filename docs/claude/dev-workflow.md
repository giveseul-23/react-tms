...
`Lang.get(key)` 는 **멱등하지 않다** — 키 미스 시 `key + "***"` 를 반환하므로, 한 문자열에 대해 **정확히 한 번만** 호출해야 한다(두 번 → `"번역문***"`, 누락 → 키 노출). 그래서 "번역을 누가 하는가"를 영역별로 고정한다.

### 라벨류 → 컴포넌트가 내부 번역 (호출자는 **키만** 넘긴다)

그리드 `headerName`, 액션 버튼 `label`, 탭 label, **팝업 `title`**, SearchFilters/FormBodyRenderer 필드 라벨 등 UI 라벨.

- 호출자(Controller/Columns/액션/`openPopup`)는 `LBL_*`/`BTN_*` **키 문자열**을 그대로 넘긴다. `Lang.get` 으로 감싸지 말 것.
- 리터럴(번역 불필요)이 필요하면 `noLang: true`(컬럼/액션) 또는 그냥 문자열을 넘긴다.

### 메시지류 → 호출자가 미리 번역 (`Lang.get(...)` 결과를 넘긴다)

`showInfoModal` / `showErrorModal` / `base.alert` / `base.confirm` / `ConfirmModal` `description`, 액션 팩토리 메시지 등 알림·확인 문구.

- 호출자가 `Lang.get("MSG_...")` 로 **번역한 문자열**을 넘긴다(컴포넌트는 raw 렌더).
- **이유**: 메시지는 placeholder 인자에 언어팩 키를 받아 조합하는 경우가 있다 — `Lang.get("MSG_VALID_INT_LEN_MAX", headerName, "10")` 처럼 호출 측에서 인자까지 합쳐 번역하는 게 자연스럽다.

### 요약

- **라벨 = 키 넣기**(컴포넌트가 번역) / **메시지 = 번역해서 넣기**(호출자가 번역).
- 어느 쪽이든 `Lang.get` 은 한 번만. 경계를 넘나들 때 이중 번역(`***`) 주의.
